...
public class ConstructibleBuilding : MonoBehaviour, CloudTerraceRealm.SaveSystem.ISaveable
{
    ...
    void Awake()
    {
        ...
        GameLog.LogVerbose($"[ConstructibleBuilding] Khởi tạo trên {gameObject.name}. Số lượng đối tượng con gốc: {transform.childCount}");
        ...
        {
            GameLog.LogVerbose($"   -> Phát hiện Mesh trực tiếp trên ROOT của {gameObject.name}. Đang trích xuất xuống con...");
            ...
            GameLog.LogVerbose("   -> Đã trích xuất xong Mesh từ ROOT xuống con.");
        }
        ...
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            GameLog.LogVerbose($"   -> Đã thu thập đối tượng con: {child.name} (Static đã tắt)");
        }
        ...
        GameLog.LogVerbose($"[ConstructibleBuilding] Chiều cao ngôi nhà: {boundsHeight}m. Lún xuống: {_initialSunkHeight}m để chừa lại chóp nhô lên: {tipOffset}m");
        ...
        if (_navObstacle != null)
        {
            if (_isInstantBuild)
            {
                ...
                GameLog.LogVerbose($"   -> Giữ nguyên NavMeshObstacle bật cho công trình xây ngay trên {gameObject.name}");
            }
            else
            {
                _navObstacle.enabled = false;
                GameLog.LogVerbose($"   -> Đã tạm thời tắt NavMeshObstacle trên {gameObject.name}");
            }
        }
        ...
        else
        {
            ...
            GameLog.LogVerbose($"   -> Đã lún _VisualContainer xuống Y = {_initialSunkHeight}. Vị trí cục bộ mới: {_visualContainer.localPosition}");
        }
    }
    ...
    void Start()
    {
        ...
        if (_constructionFencePrefab == null)
        {
            ...
            if (_constructionFencePrefab != null)
            {
                GameLog.LogVerbose($"[ConstructibleBuilding] Tự động tải thành công prefab hàng rào: {_constructionFencePrefab.name}");
            }
            #endif
        }
        ...
        if (_constructionFencePrefab != null)
        {
            ...
            GameLog.LogVerbose($"[ConstructibleBuilding] Khởi tạo hàng rào xây dựng cho {gameObject.name}. ParentScale: {parentScaleX}, GridSize: {buildingGridSize}, CellSize: {cellSize}, TargetSize: {targetSizeX}x{targetSizeZ}, LocalFenceSize: {fenceSizeX:F2}x{fenceSizeZ:F2}, FinalLocalScale: {scaleX:F2}");
        }
        else
        {
    ...
    private void CompleteConstruction()
    {
        ...
        if (_constructionFenceInstance != null)
        {
            ...
            GameLog.LogVerbose($"[ConstructibleBuilding] Đã hủy hàng rào xây dựng của {gameObject.name}");
        }
        ...
        GameLog.LogVerbose($"[ConstructibleBuilding] '{gameObject.name}' đã hoàn tất xây dựng và đi vào hoạt động!");
    }
